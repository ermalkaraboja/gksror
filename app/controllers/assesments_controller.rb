class AssesmentsController < ApplicationController
  ...
  def create
    ...
    @assesment.course_id = params[:course_id]

    respond_to do |format|
      if @assesment.save
        format.html { redirect_to course_path(:id=>params[:course_id]), notice: 'Assesment was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @assesment.update(assesment_params)
        format.html { redirect_to course_path(:id=>params[:course_id]), notice: 'Assesment was successfully updated.' }
  ...
  private
    ...
    def assesment_params
      params.require(:assesment).permit(:title, :weight)
    end
end
